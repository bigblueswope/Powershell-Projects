...
$DataSource = "$Env:systemdrive\Users\$UserName\AppData\Local\Google\Chrome\User Data\Default\History"


foreach($visited_url in Invoke-SqliteQuery -DataSource $DataSource -Query "SELECT url, datetime(last_visit_time / 1000000 + (strftime('%s', '1601-01-01')), 'unixepoch') as last_visit_time 
        FROM urls
        WHERE last_visit_time != 0		
		ORDER BY last_visit_time DESC" -As PSObject) { 
    Set-MetaData -user $UserName -hostname $hostname -type "chrome_history" -object $visited_url
     $out_json = $visited_url | ConvertTo-STJson -Compress
     $out_json

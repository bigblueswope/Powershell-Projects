...
#>
    ...
    function Get-ChromeHistory {
        ...
        $Regex = '(htt(p|s))://([\w-]+\.)+[\w-]+(/[\w- ./?%&=]*)*?'
		$Regex2 = '(htt(p|ps))://'
		$Oranges = Get-Content -Path "$Env:systemdrive\Users\$UserName\AppData\Local\Google\Chrome\User Data\Default\History"|Select-String -AllMatches $Regex
		$Oranges | ForEach-Object {$_}

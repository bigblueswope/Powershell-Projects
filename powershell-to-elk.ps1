...
{	
 Add-Member -InputObject $object -MemberType NoteProperty -Name username -Value $user
 Add-Member -InputObject $object -MemberType NoteProperty -Name hostname -Value $hostname
 Add-Member -InputObject $object -MemberType NoteProperty -Name type -Value $type
}

Function ConvertTo-STJson20([object] $item){
...
#       are quoted. Comparing to the PowerShell team's ConvertTo-STJson. Now escaping works better and more
...
# v0.9.3.3: Comparing to the PS team's ConvertTo-STJson again and they don't escape "/" alone. Undoing 0.9.2.2 change.
...
foreach($proc in Get-WmiObject -class Win32_Process | Select-Object -property ExecutablePath,CommandLine,Caption, ProcessName, Description, sessionid, processid) {
 Set-MetaData -user $me.username -hostname $hostname -type "process" -object $proc
 $proc.psobject.properties | % {if($_.Value -eq $null){$_.Value = "empty"}}
 $out_json = $proc | ConvertTo-STJson -Compress
 $out_json
...
foreach($serv in Get-WmiObject -class Win32_Service | Select-Object -property Name, Status, PathName, ServiceType, StartMode,DisplayName, Description) {
 ...
 $out_json = $serv | ConvertTo-STJson -Compress
 $out_json
 Send-StringOverTcp -DataToSend $out_json -Hostname $serverIP -Port $port
}
...
foreach($startup in Get-WmiObject -class Win32_StartupCommand | Select-Object -Property Name, User, Command, Path, Location) {
 ...
 $out_json = $startup | ConvertTo-STJson -Compress
 $out_json
 Send-StringOverTcp -DataToSend $out_json -Hostname $serverIP -Port $port
}
...
foreach($group in Get-WmiObject -class Win32_Group -Filter {localaccount = "True"} | Select-Object -Property Name,Scope,Caption,Description)   {
 ...
 $out_json = $group | ConvertTo-STJson -Compress
 $out_json
 Send-StringOverTcp -DataToSend $out_json -Hostname $serverIP -Port $port
}
...
foreach($useraccount in Get-WmiObject -class Win32_Useraccount -Filter {localaccount= "True"} | Select-Object -Property Name,Description,Disabled,Status) {
 ...
 $out_json = $useraccount | ConvertTo-STJson -Compress
 $out_json
 Send-StringOverTcp -DataToSend $out_json -Hostname $serverIP -Port $port
}
...
foreach($prefetch in Get-ChildItem -path C:\windows\prefetch\*.pf | select Name, Length, FullName, CreationTime){
    ...
    $out_json = $prefetch | ConvertTo-STJson -Compress
    $out_json
    Send-StringOverTcp -DataToSend $out_json -Hostname $serverIP -Port $port
}
...
foreach($nic in Get-WmiObject -class Win32_NetworkAdapter | select Name, ServiceName)
{
    ...
    $out_json = $nic | select Name, ServiceName, username, hostname, type | ConvertTo-STJson -Compress
    $out_json
    Send-StringOverTcp -DataToSend $out_json -Hostname $serverIP -Port $port
}
...
foreach($software in Get-WmiObject -class Win32_Product | select *)
{
    ...
    $out_json = $software | select Name, Version, Vendor, InstallDate, InstallSource, PackageName, username, hostname, type | ConvertTo-STJson -Compress
    $out_json
    Send-StringOverTcp -DataToSend $out_json -Hostname $serverIP -Port $port
}
...
foreach($loc in Get-WmiObject win32_userprofile | where {$_.special -eq $false} | select localpath, sid)
{
   ...
   foreach ($folder in $path){
       ...
       foreach($f in $file)
       {
            ...
            $hash = $hash -replace "-", ""
            Add-Member -InputObject $f -MemberType NoteProperty -Name hash -Value $hash
            Set-MetaData -u $me.username -hostname $hostname -type "md5" -object $f
            $out_json = $f | select Name, FullName, hash, username, hostname, type | ConvertTo-STJson -Compress
            $out_json
            Send-StringOverTcp -DataToSend $out_json -Hostname $serverIP -Port $port
       }
   }
